...
class ProviderUsage {
  ...
  ProviderUsage recordMetrics(
    double balance,
    DateTime now, {
    double? dailyUsage,
    double? monthlyUsage,
    bool estimateDailyFromBalance = true,
    bool estimateMonthlyFromBalance = true,
  }) {
    final today = _date(now);
    final currentMonth = today.substring(0, 7);
    final previousDaily = date == today ? this.dailyUsage : 0.0;
    final previousMonthly = month == currentMonth ? this.monthlyUsage : 0.0;
    final spent = lastBalance != null && balance < lastBalance!
        ? _money(lastBalance! - balance)
        : 0.0;
    return ProviderUsage(
      date: today,
      month: currentMonth,
      ...
      dailyUsage:
          dailyUsage ??
          (estimateDailyFromBalance
              ? _money(previousDaily + spent)
              : previousDaily),
      monthlyUsage:
          monthlyUsage ??
          (estimateMonthlyFromBalance
              ? _money(previousMonthly + spent)
              : previousMonthly),
    );
  }
